...
export function AddItemSheet({
  ...
  function addToList(libraryItemId: string) {
    start(async () => {
      const fd = new FormData()
      fd.set("listId", listId)
      fd.set("libraryItemId", libraryItemId)
      fd.set("quantity", "1")
      const res = await addToGroceryListAction(fd)
  ...
  function createAndAdd() {
    start(async () => {
      ...
      add.set("listId", listId)
      add.set("libraryItemId", res.id)
      add.set("quantity", "1")
      const r2 = await addToGroceryListAction(add)
